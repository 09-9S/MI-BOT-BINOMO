...
st.set_page_config(page_title="INFINITY PROFIT V92", layout="wide")
...
st.markdown("""
    ...
    .alert-mercado { background: #5a4a00; color: #ffd700; padding: 15px; border-radius: 10px; text-align: center; border: 1px solid #ffd700; margin-bottom: 20px; }
    .gale-warning { background: #4a0000; color: #ff4b4b; padding: 10px; border-radius: 8px; border: 1px solid red; margin-top: 10px; }
    </style>
""", unsafe_allow_html=True)

# --- 3. MEMORIA Y SEGURIDAD (ANTIFALLO) ---
...
if st.session_state.loss >= 4:
    st.error("⛔ STOP LOSS ALCANZADO (4/4)")
...
with st.sidebar:
    ...
    saldo_cta = st.number_input("Saldo ($):", min_value=0, value=10)
...
# --- 6. PESTAÑAS (OPTIMIZADAS PARA CELULAR) ---
tab1, tab2 = st.tabs(["📉 BINARIAS", "🏛️ FOREX / MT5"])

with tab1:
    col1, col2 = st.columns([1, 1.2])
    with col1:
        # BLOQUE DE CÁMARA CON PROTECCIÓN CONTRA ERRORES
        try:
            st.camera_input("Scanner", key="cam_v92")
        except:
            st.warning("⚠️ Cámara no disponible en este dispositivo.")
        ...
        if st.session_state.sig_bin:
            ...
            st.markdown(f"""
                <div class="signal-card" style="background:{color};">
                    <p>{divisa_actual} | {s["in"]}</p>
                    <h1>{s["tipo"]}</h1>
                    <h2 style="color:#ffd700;">CIERRE: {s["out"]}</h2>
                    {"<div class='gale-warning'>⚠️ NO USAR GALE ($<100)</div>" if saldo_cta <= 100 else ""}
                </div>
            """, unsafe_allow_html=True)
...
# --- 7. GRÁFICA (AJUSTE DE CARGA PARA MÓVIL) ---
st.divider()
st.components.v1.html(f"""
    <div id="tv_v92" style="height:500px; border-radius:15px; overflow:hidden;"></div>
    <script type="text/javascript" src="https://s3.tradingview.com/tv.js"></script>
    <script type="text/javascript">
    new TradingView.widget({{"width":"100%","height":500,"symbol":"{DIVISAS[divisa_actual]}","interval":"5","theme":"dark","locale":"es","container_id":"tv_v92"}});
    </script>
""", height=520)